...
// Add a new teacher row – preserve existing selections and avoid resetting state
function addTeacherRow() {
    // Directly append a new empty teacher entry to the in‑memory state.
    // This avoids re‑reading the table (which would drop hidden values
    // like classTeacherSection) and therefore prevents rows from
    // disappearing or losing their selections.
    state.teachers.push({
        id: '',
        name: '',
        classTeacherSubject: '',
    ...
    // Re‑render the UI so the new row appears, preserving existing rows.
    renderTeacherMasterTable();

    // Keep the dashboard summary (teacher count) in sync.
    updateSetupSummary();
}
